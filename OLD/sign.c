                          ...
                          uint32_t addr_idx, const uint32_t tree_addr[8])
{
    ...
    set_type(wots_addr, WOTS_HASH);
    set_type(wots_pk_addr, WOTS_PK);
                             ...
                             const unsigned char *seed)
{
    /* We do not need the auth path in key generation, but it simplifies the
       ...
       in one function. */
    unsigned char auth_path[(FIPS205_H/ FIPS205_D) * FIPS205_N];
    uint32_t top_tree_addr[8] = {0};

    set_layer_addr(top_tree_addr, FIPS205_D - 1);
    set_type(top_tree_addr, TREE);

    /* Initialize SK_SEED, SK_PRF and PUB_SEED from seed. */
    memcpy(sk, seed, 3 * FIPS205_N);

    memcpy(pk, sk + 2* FIPS205_N, FIPS205_N);
    ...
    treehash(sk + 3* FIPS205_N, auth_path, sk, sk + 2* FIPS205_N, 0, 0, (FIPS205_H / FIPS205_D),
             wots_gen_leaf, top_tree_addr);

    memcpy(pk + FIPS205_N, sk + 3 * FIPS205_N, FIPS205_N);
                          ...
                          const uint8_t *m, size_t mlen, const uint8_t *sk)
{
    ...
    memcpy(optrand, sk + 2 * SPX_N, SPX_N);
    ...
    )
{
    ...
    uint32_t tree_addr[32] = { 0 }, wots_addr[32] = { 0 };
    set_type(wots_addr, SPX_ADDR_TYPE_WOTS);
    set_type(tree_addr, SPX_ADDR_TYPE_HASHTREE);
